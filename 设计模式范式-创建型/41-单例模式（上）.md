...
那我们该怎么解决这个问题呢？实际上，要想解决这个问题也不难，我们**==只需要把对象级别的锁，换成类级别的锁就可以了==**。让所有的对象都共享同一把锁。这样就避免了不同对象之间同时调用 log() 函数，而导致的日志覆盖问题。具体的代码实现如下所示：
...
- <u>构造函数需要是 private 访问权限的，这样才能避免外部通过 new 创建实例；</u>
- <u>考虑对象创建时的线程安全问题；</u>
- <u>考虑是否支持延迟加载；</u>
- <u>考虑 getInstance() 性能是否高（是否加锁）。</u>
